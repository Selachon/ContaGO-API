...
export async function processXmlForAccounting(xmlBuffer: Buffer): Promise<XmlCausacionData> {
  ...
  if (!xmlData.issuerNit && lineItems.length === 0) {
    throw new Error("El XML no es una factura/documento procesable o no se pudo leer.");
  }
...
export async function processXmlBatch(
  ...
  // Expandir: descomprimir los ZIP y recolectar todos los XML
  const xmlFiles: { name: string; buffer: Buffer }[] = [];
  for (const f of files) {
    if (/\.zip$/i.test(f.name)) {
      const zip = await JSZip.loadAsync(f.buffer);
      for (const entry of Object.values(zip.files)) {
        if (entry.dir || !/\.xml$/i.test(entry.name)) continue;
        const buf = await entry.async("nodebuffer");
        xmlFiles.push({ name: entry.name.split("/").pop() || entry.name, buffer: buf });
      }
  ...
  console.log(`[SiigoAccounting] Lote: ${files.length} archivo(s) → ${xmlFiles.length} XML`);
  ...
  for (const xf of xmlFiles) {
    try {
      ...
      results.push({ fileName: xf.name, ok: true, xml, profile });
    } catch (e) {
      results.push({
        fileName: xf.name,
        ok: false,
